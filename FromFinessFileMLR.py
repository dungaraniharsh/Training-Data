...
def calculate_r2_q2(predicted, actual):
    residual = actual - predicted
    ss_res = np.sum(residual ** 2)
    ss_tot = np.sum((actual - np.mean(actual)) ** 2)
    r2 = 1 - (ss_res / ss_tot)
    q2 = 1 - (np.var(residual) / np.var(actual))
    return r2, q2
...
class FitnessResults:
    ...
    def cv_predict(self, model, set_x, set_y):
        yhat = np.empty_like(set_y)
        for idx in range(set_y.shape[0]):
            train_x = np.delete(set_x, idx, axis=0)
            train_y = np.delete(set_y, idx, axis=0)
            try:
